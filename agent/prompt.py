...
class ToolsPrompt:
    def __init__(self):
        ...
        1. [路线规划], {{"args_1":"str, 路线的起点", "args_2", "str, 路线的终点"}} # 只考虑起点终点
        2. [物理地址经纬度转换], {{"args_1": "str, 需要转换的物理地址"}} # 只考虑地址
        
        输出的结构必须和```包裹的JSON结构相同：
        ```
        {{
            "tool": "路线规划", # 必须是可选的工具的名称
